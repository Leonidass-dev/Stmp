...
public class Server3 implements ServerInterface {
    ...
    public void start() {
        ...
        System.out.println("Server3 başlatılıyor...");
        ...
        CompletableFuture.runAsync(() -> {
            try (ServerSocket adminSocket = new ServerSocket(ADMIN_PORT)) {
                System.out.println("Server3 admin bağlantıları için dinleniyor: " + ADMIN_PORT);
                while (isRunning) {
        ...
        CompletableFuture.runAsync(() -> {
            try (ServerSocket clientSocket = new ServerSocket(CLIENT_PORT)) {
                System.out.println("Server3 client bağlantıları için dinleniyor: " + CLIENT_PORT);
                while (isRunning) {
    ...
    private void connectToPeer(int peerId, String host, int port) {
        try {
            ...
            System.out.println("Server3: Peer" + peerId + "'e bağlantı başarılı");
        } catch (IOException e) {
            System.err.println("Server3: Peer" + peerId + "'e bağlanılamadı: " + e.getMessage());
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("Server3 kapatılıyor...");
            server.stop();
        }));
