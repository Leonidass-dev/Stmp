...
public class Server1 implements ServerInterface {
    ...
    public void start() {
        ...
        System.out.println("Server1 başlatılıyor...");
        ...
        CompletableFuture.runAsync(() -> {
            try (ServerSocket adminSocket = new ServerSocket(ADMIN_PORT)) {
                System.out.println("Server1 admin bağlantıları için dinleniyor: " + ADMIN_PORT);
                while (isRunning) {
        ...
        CompletableFuture.runAsync(() -> {
            try (ServerSocket clientSocket = new ServerSocket(CLIENT_PORT)) {
                System.out.println("Server1 client bağlantıları için dinleniyor: " + CLIENT_PORT);
                while (isRunning) {
    ...
    private void connectToPeers() {
        if (config.getFaultTolerance() >= 1) {
            connectToPeer(2, "localhost", 5002);
        }
    ...
    private void connectToPeer(int peerId, String host, int port) {
        try {
            ...
            System.out.println("Server1: Peer" + peerId + "'e bağlantı başarılı");
        } catch (IOException e) {
            System.err.println("Server1: Peer" + peerId + "'e bağlanılamadı: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            while (isRunning && !peerSocket.isClosed()) {
                ...
                if (message != null && message.getType() == Message.Type.SYNC) {
                    ...
                    System.out.println("Server1: Peer" + peerId + "'den senkronizasyon alındı");
                }
            }
        } catch (IOException e) {
            System.err.println("Server1: Peer" + peerId + " bağlantısı koptu: " + e.getMessage());
    ...
    private void sendToPeer(int peerId, Message message) {
        ...
        if (peerSocket != null && !peerSocket.isClosed()) {
            try {
                ...
                System.out.println("Server1: Peer" + peerId + "'e senkronizasyon gönderildi");
            } catch (IOException e) {
                System.err.println("Server1: Peer" + peerId + "'e mesaj gönderilemedi: " + e.getMessage());
                peerConnections.remove(peerId);
            }
        }
    }
